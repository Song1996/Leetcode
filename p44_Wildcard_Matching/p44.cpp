...
public:
    ...
    string s; string p;
    bool isMatch(string xs, string xp) {
        s = xs; p = xp;
        ...
        printf("entered\n");*/
        return myMatch(0,0,s.length(),p.length())==1;
    }
    int myMatch(int x, int y, int lx, int ly) {
        pair<int, int> index; index.first = x; index.second = y;
        if(dict[x][y] != 0) return dict[x][y];
        if(x == lx && y == ly) dict[x][y] = 1;
        else if(x == lx) {
            if(p[y]!='*') dict[x][y] = 2;
            else dict[x][y] = myMatch(x, y+1, lx, ly);
        }
        else if(y == ly) dict[x][y] = 2;
        else if(p[y]=='?') dict[x][y] = myMatch(x+1, y+1, lx, ly);
        else if(p[y]!='*') {
            if(p[y]==s[x]) dict[x][y] = myMatch(x+1, y+1, lx, ly);
            else dict[x][y]= 2;
        }
        else {
            if(myMatch(x, y+1, lx, ly)==1) dict[x][y] = 1;
            for(int i = 1; x+i <= s.length(); i++){
                if(myMatch(x+i, y+1, lx, ly)==1) dict[x][y] = 1;
            }
        }
